defmodule Lodestar.GraphCache do
  @moduledoc """
  ...
  def handle_call({:fold, port}, _from, %{fold: nil} = st),
    do: (f = Lodestar.Threads.fold_triples(port); {:reply, f, %{st | fold: f, port: port}})
  ...
  def handle_info({:commit, "board"}, st) do
    me = self()
    port = st.port
    Task.start(fn -> send(me, {:set, Lodestar.Threads.fold_triples(port)}) end)
    {:noreply, st}
  end
